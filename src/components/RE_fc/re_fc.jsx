...
import VistaPreviaPDFFactura from "../RE_fc/VistaPreviaPDFFactura"; 
export default function RE_fc() {
  ...
  const construirParametros = () => ({
    ...
    bloque_id: parseInt(formReporte.bloque_id),
    espacio_id: parseInt(formReporte.espacio_id),
    almacen_id: parseInt(formReporte.almacen_id),
    municipio_id: parseInt(formReporte.municipio_id),
    producto_id: parseInt(formReporte.producto_id),
    producto_categoria_id: parseInt(formReporte.producto_categoria_id),
  ...
  const generarVistaPreviaPDF = () => {
    ...
    axios.post("/v2/report/factura", datos, {
      headers: {
  ...
  const imprimirPDF = () => {
    ...
    axios.post("/v2/report/factura", datos, {
      ...
      link.href = url;
      link.setAttribute("download", `reporte_factura.pdf`);
  ...
  {[
    ...
    <Grid item xs={6} key={name}>
      ...
      <Box mt={4} display="flex" gap={2}>
        ...
        <Button variant="outlined" onClick={imprimirPDF}>Imprimir PDF</Button>
      </Box>

      {previewUrl && (
        <Box mt={4}>
          <Typography variant="h6" gutterBottom>Vista previa del PDF</Typography>
          <VistaPreviaPDFFactura url={previewUrl} />
        </Box>
      )}
