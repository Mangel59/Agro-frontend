...
const columns = [
  ...
  { field: 'area', headerName: 'Área', width: 150, type: 'number' },
  { field: 'comuna', headerName: 'Comuna', width: 150, type: 'string' },
...
export default function GridSede(props) {
  const handleRowSelection = (selectionModel) => {
    if (selectionModel.length > 0) {
      const selectedRow = props.sedes.find((row) => row.id === selectionModel[0]);
  ...
  return (
    <DataGrid
      rows={props.sedes || []}  // Si `props.sedes` es undefined, pasa un array vacío
      getRowId={(row) => row.id} 
