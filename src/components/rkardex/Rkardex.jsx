...
import MessageSnackBar from '../MessageSnackBar';
import FormKardex from "./FormRkardex";
import GridKardex from "./GridRkardex";
import ComboBox from '/src/components/ComboBox.jsx';
...
export default function Kardex() {
  ...
  // Funciones que serán pasadas a ComboBox como props para manejar los cambios individuales
  const handleAlmacenChange = (almacen) => {
    setSelectedRow((prev) => ({ ...prev, kar_almacen_id: almacen }));
  };

  const handleProduccionChange = (produccion) => {
    setSelectedRow((prev) => ({ ...prev, kar_produccion_id: produccion }));
  };

  const handleMovimientoChange = (movimiento) => {
    setSelectedRow((prev) => ({ ...prev, kar_tipo_movimiento_id: movimiento }));
  };

  const handleDescripcionChange = (descripcion) => {
    setSelectedRow((prev) => ({ ...prev, kar_descripcion: descripcion }));
  };

  const handleEstadoChange = (estado) => {
    setSelectedRow((prev) => ({ ...prev, kar_estado: estado }));
  };
  ...
  return (
    <div style={{ height: '100%', width: '100%' }}>
      ...
      <ComboBox
        ...
        onProduccionChange={handleProduccionChange}
