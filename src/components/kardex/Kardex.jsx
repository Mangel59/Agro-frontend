...
export default function Kardex() {
  ...
  React.useEffect(() => {
    fetchData('/almacen.json', setAlmacen);
    fetchData('/produccion.json', setProduccion);
    fetchData('/tipomovimiento.json', setTipoMovimiento);
    fetchData('/estado.json', setEstado);
  }, []);

  // Función para agregar un nuevo almacén
  const addKardex = (newData) => {
    const newKardex = { ...newData, id: Date.now() };  // Genera un ID único usando Date.now()
    setKardex((prevData) => [...prevData, newAlmacen]);
    setMessage({ open: true, severity: "success", text: "Almacén creado con éxito!" });
  };

  // Función para actualizar un almacén
  const updateKardex = (updatedData) => {
    setKardex((prevData) =>
  ...
  const deleteKardex = (id) => {
    setKardex((prevData) => prevData.filter((item) => item.id !== id));
