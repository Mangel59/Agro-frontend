...
export default function ReportOrdenCompraViewer() {
  ...
  const [formatoReporteOrdenCompra, setFormatoReporteOrdenCompra] = useState('PDF'); // Estado para el formato del reporte
  ...
  const generarReporteOrdenCompraPDF = () => {
    const doc = new jsPDF();
  
    // Título
    doc.setFontSize(20);
    doc.text('ReporteOrdenCompra Detallado', 20, 20);
  ...
  const generarReporteOrdenCompraCSV = () => {
  ...
  const handleGenerarReporteOrdenCompra = () => {
    setOpenDialog(true);
  };
  ...
  const handleFormatoSeleccionado = () => {
    if (formatoReporteOrdenCompra === 'PDF') {
      generarReporteOrdenCompraPDF();
    } else if (formatoReporteOrdenCompra === 'CSV') {
      generarReporteOrdenCompraCSV();
    }
  ...
  return (
    <Container maxWidth="xl" sx={{ mt: 4 }}>
      <Grid container spacing={2}>
        ...
        <Grid item xs={12}>
          <Button variant="contained" onClick={handleGenerarReporteOrdenCompra} sx={{ mt: 2 }}>
            Generar Informe
          </Button>
        </Grid>
      </Grid>
      ...
      <Dialog open={openDialog} onClose={handleCloseDialog}>
        ...
        <DialogContent>
          <FormControl fullWidth>
            ...
            <Select value={formatoReporteOrdenCompra} onChange={(e) => setFormatoReporteOrdenCompra(e.target.value)}>
